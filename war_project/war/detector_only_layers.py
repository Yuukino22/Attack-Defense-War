...
class Det_Training():
    ...
    def train(self, model, trainset, valset, testset, device, epoches=8):
        model.to(device)
        model.train()
        b_size = 100
        ...
        for epoch in range(epoches):  # loop over the dataset multiple times
            ...
            for inputs, labels in train_data:
                ...
                optimizer.zero_grad()
                outputs = model(self.ori_model.layers(inputs))
        ...
        for inputs, labels in valloader:
            ...
            for i in range(len(inputs)):
                ...
                output = model(self.ori_model.layers(pert_input))
...
if __name__ == "__main__":
    ...
    model = method.train(method.model, trainset, valset, testset, device)
    torch.save(model.state_dict(), "/Volumes/test/maestro-class/playground/war_phase/tasks/war_defense/war_defense-Jason-01/war_project-model-detector-only-layers.pth")
